...
export default function HomeScreen() {
  ...
  const getWeeklyInsights = useHistoryStore((s) => s.getWeeklyInsights);
  const recentProducts = products.slice(0, 6);

  const weeklyInsights = useMemo(() => getWeeklyInsights(), [products]);
    ...
    <View className="flex-1 bg-slate-50">
      ...
      <SafeAreaView className="flex-1" edges={['top']}>
        ...
        >
          ...
          {/* Weekly Insights */}
          {weeklyInsights.scansThisWeek > 0 && (
            <Animated.View entering={FadeInDown.delay(175).springify()} className="px-6 mt-8">
              <View className="flex-row items-center justify-between mb-4">
                <Text className="text-lg font-bold text-slate-900">This Week's Insights</Text>
                <View className="flex-row items-center">
                  <Sparkles size={16} color={COLORS.brandGreen} />
                  <Text className="text-teal-600 font-semibold text-sm ml-1">
                    {weeklyInsights.scansThisWeek} scans
                  </Text>
                </View>
              </View>

              {/* Health Score Card */}
              <View className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 mb-3">
                <View className="flex-row items-center justify-between mb-4">
                  <Text className="text-slate-900 font-semibold text-base">Health Breakdown</Text>
                  {weeklyInsights.improvementFromLastWeek !== 0 && (
                    <View className={cn(
                      "flex-row items-center px-2 py-1 rounded-full",
                      weeklyInsights.improvementFromLastWeek > 0 ? "bg-emerald-50" : "bg-red-50"
                    )}>
                      <TrendingUp
                        size={12}
                        color={weeklyInsights.improvementFromLastWeek > 0 ? COLORS.safeGreen : COLORS.alertRed}
                        style={{ transform: [{ rotate: weeklyInsights.improvementFromLastWeek > 0 ? '0deg' : '180deg' }] }}
                      />
                      <Text className={cn(
                        "text-xs font-semibold ml-1",
                        weeklyInsights.improvementFromLastWeek > 0 ? "text-emerald-600" : "text-red-600"
                      )}>
                        {Math.abs(weeklyInsights.improvementFromLastWeek).toFixed(0)}%
                      </Text>
                    </View>
                  )}
                </View>
                ...
                <View className="gap-3">
                  <View>
                    <View className="flex-row items-center justify-between mb-2">
                      <View className="flex-row items-center">
                        <CheckCircle size={14} color={COLORS.safeGreen} />
                        <Text className="text-slate-600 text-sm ml-2">Safe</Text>
                      </View>
                      <Text className="text-slate-900 font-semibold text-sm">
                        {weeklyInsights.safePercentage.toFixed(0)}%
                      </Text>
                    </View>
                    <View className="h-2 bg-slate-100 rounded-full overflow-hidden">
                      <View
                        ...
                        style={{
                          width: `${weeklyInsights.safePercentage}%`,
                          backgroundColor: COLORS.safeGreen
                        }}
                      />
                    </View>
                  ...
                  <View>
                    <View className="flex-row items-center justify-between mb-2">
                      <View className="flex-row items-center">
                        <AlertTriangle size={14} color={COLORS.cautionYellow} />
                        <Text className="text-slate-600 text-sm ml-2">Caution</Text>
                      </View>
                      <Text className="text-slate-900 font-semibold text-sm">
                        {weeklyInsights.cautionPercentage.toFixed(0)}%
                      </Text>
                    </View>
                    <View className="h-2 bg-slate-100 rounded-full overflow-hidden">
                      <View
                        className="h-full rounded-full"
                        style={{
                          width: `${weeklyInsights.cautionPercentage}%`,
                  ...
                  <View>
                    <View className="flex-row items-center justify-between mb-2">
                      <View className="flex-row items-center">
                        <XCircle size={14} color={COLORS.alertRed} />
                        <Text className="text-slate-600 text-sm ml-2">Warning</Text>
                      </View>
                      <Text className="text-slate-900 font-semibold text-sm">
                        {weeklyInsights.warningPercentage.toFixed(0)}%
                      </Text>
                    </View>
                    <View className="h-2 bg-slate-100 rounded-full overflow-hidden">
                      <View
                        className="h-full rounded-full"
                        style={{
                          width: `${weeklyInsights.warningPercentage}%`,
              ...
              {/* Most Common Category & Top Flags */}
              <View className="flex-row gap-3">
                {weeklyInsights.mostCommonCategory && (
                  <View className="flex-1 bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                    <Text className="text-slate-500 text-xs font-medium uppercase">Top Category</Text>
                    <Text className="text-slate-900 font-bold text-lg mt-2">
                      {CATEGORY_LABELS[weeklyInsights.mostCommonCategory]}
                    </Text>
                  </View>
                )}

                {weeklyInsights.topFlaggedIngredients.length > 0 && (
                  <View className="flex-1 bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                    <Text className="text-slate-500 text-xs font-medium uppercase">Most Flagged</Text>
                    <Text className="text-slate-900 font-bold text-base mt-2 capitalize" numberOfLines={1}>
                      {weeklyInsights.topFlaggedIngredients[0].name}
                    </Text>
                    <Text className="text-slate-400 text-xs mt-1">
                      {weeklyInsights.topFlaggedIngredients[0].count}x this week
                    </Text>
                  </View>
                )}
              </View>
